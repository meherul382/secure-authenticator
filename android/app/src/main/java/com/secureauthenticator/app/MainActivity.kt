...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState:Bundle?){super.onCreate(savedInstanceState);setContentView(R.layout.activity_main);list=findViewById(R.id.accountList);val key=MasterKey.Builder(this).setKeyScheme(MasterKey.KeyScheme.AES256_GCM).build();store=EncryptedSharedPreferences.create(this,"secure_accounts",key,EncryptedSharedPreferences.PrefKeyEncryptionScheme.AES256_SIV,EncryptedSharedPreferences.PrefValueEncryptionScheme.AES256_GCM);load();findViewById<Button>(R.id.addButton).setOnClickListener{dialog()};findViewById<Button>(R.id.importButton).setOnClickListener{uriDialog()};render()}
